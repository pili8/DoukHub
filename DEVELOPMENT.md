...
1. **以官方文档为准**：优先参考 TikTokDownloader 和 XHS-Downloader 的官方文档（TTD docs: http://127.0.0.1:5555/docs）
...
- `POST /douyin/share` - 解析短链接，获取完整 URL
- `POST /douyin/account` - 获取账号作品数据（用 count=1 优化性能）
- `POST /douyin/detail` - 获取单个作品数据
- `POST /douyin/search/user` - 用户搜索
- TikTok 对应 `/tiktok/*` 端点
